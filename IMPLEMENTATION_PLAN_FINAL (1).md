...
- "Task so far" is compact: one line per step (`step 3: click 'Apply filters' → ok`), not full snapshots. Only the _latest_ snapshot goes in the prompt.
...
routes:
  ...
  - {
      pattern: "/checkout",
      purpose: "checkout; contains payment fields",
      sensitive: true,
    }
  - {
